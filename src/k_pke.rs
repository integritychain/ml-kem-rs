...
use super::auxiliary_algorithms::g;
...
pub fn k_pke_key_gen<const K: usize>(ek_pke: &mut [u8], dk_pke: &mut [u8]) {
    ...
    // let mut dk_pke = [0u8; 384 * K]; // TODO these two should be the function parms ??
    ...
    for i in 0..K {
        for j in 0..K {
            // TODO a_hat[i][j] = sample_ntt::<3329>(xof(&d, i.try_into().unwrap(), j.try_into().unwrap()));
        }
    }
    let mut s: [[Z256; 256]; K] = [[0; 256]; K];
    for i in 0..K {
        // TODO s[i] = sample_poly_cbd::<2, 3379>(&prf::<2>(&sigma, n));
        n += 1;
    }
    let mut e: [[Z256; 256]; K] = [[0; 256]; K];
    for i in 0..K {
        //TODO e[i] = sample_poly_cbd::<2, 3379>(&prf::<2>(&sigma, n));
        n += 1;
    }

    let mut s_hat: [[Z256; 256]; 2] = [[0; 256]; 2];
    for i in 0..2 {
        // TODO s_hat[i] = ntt(&s[i]);
    }
    let mut e_hat: [[Z256; 256]; 2] = [[0; 256]; 2];
    for i in 0..2 {
        // TODO e_hat[i] = ntt(&e[i]);
    }
    ...
    let mut ek = Vec::new();
    for i in 0..2 {
        // TODO let t = byte_encode::<12, 3379>(t_hat[i]);
        // TODO ek.extend(t);
    }
    ek.extend(rho);
    ...
    let mut dk = Vec::new();
    for i in 0..2 {
        // TODO let t = byte_encode::<12, 3379>(s[i]);
        // TODO dk.extend(t);
    }
