...
pub fn ntt(integer_array: &[Z256; 256]) -> [Z256; 256] {
    ...
    let mut array_f_hat = [Z256(0); 256];
    array_f_hat.copy_from_slice(integer_array);
    // 2: k ← 1
    let mut k = 1;
    ...
    for len in [128, 64, 32, 16, 8, 4, 2] {
        ...
        for start in (0..256).step_by(2 * len) {
            ...
            for j in start..(start + len) {
                ...
                let t = (zeta * (array_f_hat[j + len]).get_u32()) % Q;
                // 9: f_hat[ j + len] ← f_hat [ j] − t
                array_f_hat[j + len].set_u16((Q + array_f_hat[j].get_u32() - t) % Q);
                // 10: f_hat[ j] ← f_hat[ j] + t
                array_f_hat[j].set_u16(((array_f_hat[j]).get_u32() + t) % Q);
            } // 11: end for
        } // 12: end for
    } // 13: end for
    // 14: return f_hat
...
pub fn ntt_inv(f_hat: &[Z256; 256]) -> [Z256; 256] {
    // Input: array fˆ ∈ Z256           ▷ the coeffcients of input NTT representation
    // Output: array f ∈ Z256           ▷ the coeffcients of the inverse-NTT of the input
...
pub fn multiply_ntts(f_hat: &[Z256; 256], g_hat: &[Z256; 256]) -> [Z256; 256] {
    ...
    // Output: An array h ∈ Z^{256}_q                                   ▷ the coeffcients of the product of the inputs
